...
void CDownLoadThread::GetDownLoadStatus(tagDownLoadStatus & DownLoadStatus)
{
	...
	case enDownLoadStatus_DownLoadIng:
		{
			...
			_snwprintf(DownLoadStatus.szStatus, sizeof(DownLoadStatus.szStatus),
	...
	case enDownLoadStatus_Fails:
		{
			DownLoadStatus.wProgress=0;
			_snwprintf(DownLoadStatus.szStatus, sizeof(DownLoadStatus.szStatus), TEXT("下载失败，错误号：%ld"), m_DownLoadResult);
			break;
		}
	default: 
		{
	...
	try
	{
		if (m_bPreparative==true)
		{
			...
			else 
			{
				...
				//移动文件
				_snwprintf(m_szLocalFile, sizeof(m_szLocalFile), TEXT("%s\\%s"), m_pDownLoadRequest->szLocalPath, m_pDownLoadRequest->szFileName);
...
TCHAR szDownBuffer[4096];
	try
	{
		...
		//本地文件
		_snwprintf(m_szTempFile, sizeof(m_szTempFile), TEXT("%s\\%s.DTM"), m_pDownLoadRequest->szLocalPath, m_pDownLoadRequest->szFileName);
