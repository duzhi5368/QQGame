...
public:
    ...
    //构造函数
    CTempldateHelper(REFGUID Guid, DWORD dwVersion, LPCTSTR pszModuleDll, LPCTSTR pszCreateProc);
    //析构函数
...
//构造函数
template <typename IModeluInterface>
CTempldateHelper<IModeluInterface>::CTempldateHelper(REFGUID Guid, DWORD dwVersion, LPCTSTR pszModuleDll, LPCTSTR pszCreateProc) : m_dwVersion(dwVersion), m_Guid(Guid)
{
    //辅助变量
    m_szDescribe[0] = 0;
    ...
    //组件属性
    lstrcpyn(m_szCreateProc, pszCreateProc, CountArray(m_szCreateProc));
...
bool CTempldateHelper<IModeluInterface>::CreateInstance()
{
    ...
    try
    {
        ...
        //寻找函数
        USES_CONVERSION;
        ModuleCreateProc * CreateProc = (ModuleCreateProc *)GetProcAddress(m_hDllInstance, W2A(m_szCreateProc));
        if (CreateProc == NULL)
        {
