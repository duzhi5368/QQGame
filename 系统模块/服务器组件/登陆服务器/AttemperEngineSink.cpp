...
bool __cdecl CAttemperEngineSink::OnAttemperEngineStart(IUnknownEx * pIUnknownEx)
{
	...
	//中心连接
    //显式地将中心服务器的地址设置为127.0.0.1
    _tcscpy_s(m_pInitParamter->m_szCenterServerAddr, ARRAYSIZE(m_pInitParamter->m_szCenterServerAddr), SERVER_ADDR);
    USES_CONVERSION;
	DWORD dwServerIP = inet_addr(W2A(m_pInitParamter->m_szCenterServerAddr));
...
bool __cdecl CAttemperEngineSink::OnEventTimer(DWORD dwTimerID, WPARAM wBindParam)
{
	...
	case IDI_CONNECT_CENTER_SERVER:		//连接中心服务器
		{
			//发起连接
			DWORD dwServerIP=inet_addr(CStringA(CStringW(m_pInitParamter->m_szCenterServerAddr)));
				...
				case DTP_COMPUTER_ID:		//机器标识
					{
						...
						if (DataDescribe.wDataSize==sizeof(tagClientSerial))
						{
							...
							for (INT i=0;i<sizeof(tagClientSerial)/sizeof(BYTE);i++)
							{
								...
								wsprintf(&LogonByAccounts.szComputerID[i*2],TEXT("%02X"),*(pcbByte+i));
							}
						}
				...
				case DTP_COMPUTER_ID:		//机器标识
					{
						...
						if (DataDescribe.wDataSize==sizeof(tagClientSerial))
						{
							...
							for (INT i=0;i<sizeof(tagClientSerial)/sizeof(BYTE);i++)
							{
                                ...
                                wsprintf(&RegisterAccounts.szComputerID[i * 2], TEXT("%02X"), *(pcbByte + i));
							}
						}
	...
	case SUB_GP_USER_UPLOAD_FACE:		//上传头像
		{
			...
			try
			{
				...
				_snwprintf(szFileName, CountArray(szFileName), TEXT("%s\\UploadFile_%ld.zip"), theApp.m_szDirWork, pUploadCustomFace->dwUserID);
				BOOL bSuccess=FALSE;
