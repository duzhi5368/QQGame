...
bool CGameService::LoadGameServiceModule(LPCTSTR pszModuleName)
{
	...
	try
	{
		...
		if (m_hDllInstance==NULL) 
		{
			...
			_snwprintf(szDescribe, sizeof(szDescribe), TEXT("“%s”模块加载失败"), pszModuleName);
		...
		{
			TCHAR szDescribe[256]=TEXT("");
			_snwprintf(szDescribe, sizeof(szDescribe), TEXT("“%s”创建函数不存在"), pszModuleName);
			ShowErrorMessasge(szDescribe,TraceLevel_Exception);
			throw 0;
		}
		...
		{
			TCHAR szDescribe[256]=TEXT("");
			_snwprintf(szDescribe, sizeof(szDescribe), TEXT("“%s”游戏管理接口创建失败"), pszModuleName);
			ShowErrorMessasge(szDescribe,TraceLevel_Exception);
			throw 0;
		}
