...
bool __cdecl CDataBaseSink::OnDataBaseEngineStart(IUnknownEx * pIUnknownEx)
{
	...
	try
	{
		//连接用户数据库
        //暂且先将数据库用户名和密码固化
        _tcscpy(m_pInitParamter->m_szUserDataBaseAddr, DB_SERVER_ADDR);
        _tcscpy(m_pInitParamter->m_szUserDataBaseUser, DB_USER);
        _tcscpy(m_pInitParamter->m_szUserDataBasePass, DB_PASSWORD);
...
LONG CDataBaseSink::SPLogonByAccounts(LPCTSTR pszAccounts, LPCTSTR pszPassword, DWORD dwClientIP, LPCTSTR pszComputerID)
{
	...
	_snwprintf(szClientIP, sizeof(szClientIP), TEXT("%d.%d.%d.%d"), pClientIP[0], pClientIP[1], pClientIP[2], pClientIP[3]);
