...
{
	TCHAR szRuleUrl[128];
	_snwprintf(szRuleUrl, sizeof(szRuleUrl), TEXT("http://221.234.25.72/GameRule.asp?KindID=%ld"), m_UserListView.m_wKindID);
...
void CGameFrameControl::OnNMRclickUserList(NMHDR * pNMHDR, LRESULT * pResult)
{
	...
	if (pListNotify->iItem!=-1)
	{
		...
		{
			_snwprintf(szBuffer, sizeof(szBuffer), TEXT("用户名：%s [ %s ]"), m_pUserData->szName, pszMemberOrder[m_pUserData->cbMemberOrder]);
		}
		else _snwprintf(szBuffer, sizeof(szBuffer), TEXT("用户名：%s"), m_pUserData->szName);
		UserInfoMenu.AppendMenu(0,0,szBuffer);

		//用户 ID
		_snwprintf(szBuffer, sizeof(szBuffer), TEXT("游戏 ID：%ld"), m_pUserData->dwGameID);
		UserInfoMenu.AppendMenu(0,0,szBuffer);

		//用户经验
		_snwprintf(szBuffer, sizeof(szBuffer), TEXT("经验值：%ld"), m_pUserData->lExperience);
		UserInfoMenu.AppendMenu(0,0,szBuffer);

		//用户积分
		_snwprintf(szBuffer, sizeof(szBuffer), TEXT("积分：%ld  总局数：%ld  逃跑率：%5.2f%%"), m_pUserData->lScore, lPlayCount, dFleeRate);
		UserInfoMenu.AppendMenu(0,0,szBuffer);
		...
		{
			_snwprintf(szBuffer, sizeof(szBuffer), TEXT("与 [ %s ] 聊天"), m_pUserData->szName);
			UserInfoMenu.AppendMenu(0,IDM_UM_SET_CHAT,szBuffer);
		}
		if (dwCurUserID==m_pUserData->dwUserID)
		{
			_snwprintf(szBuffer, sizeof(szBuffer), TEXT("取消与 [ %s ] 的聊天"), m_pUserData->szName);
		...
		if ((pMeUserData!=m_pUserData)&&(pMeUserData->cbUserStatus!=US_LOOKON)&&(m_pUserData->wChairID==pMeUserData->wChairID))
		{
			...
			//允许旁观
			_snwprintf(szBuffer, sizeof(szBuffer), TEXT("允许 [ %s ] 观看您游戏"), m_pUserData->szName);
			UserInfoMenu.AppendMenu(0,IDM_UM_ALLOW_LOOKON,szBuffer);

			//禁止旁观
			_snwprintf(szBuffer, sizeof(szBuffer), TEXT("禁止 [ %s ] 观看您游戏"), m_pUserData->szName);
		...
		if ( pServerAttribute->wChairCount <= MAX_CHAIR_NORMAL || m_pIClientKernel->GetMeUserInfo()->cbMasterOrder > 0 )
		{
			...
			if (m_pIClientKernel->GetMeUserInfo()->dwUserID!=m_pUserData->dwUserID)
			{
				...
				//插入菜单
				_snwprintf(szBuffer, sizeof(szBuffer), TEXT("把 [ %s ] 踢出游戏房间"), m_pUserData->szName);
