...
BOOL CSystemOptionDlg::OnInitDialog()
{
	...
	//显式地将数据库用户名和密码设置成固定的值, by zhangyl
	//_tcscpy(InitParamter.m_szServerDataBaseAddr, _T("127.0.0.1"));
	//_tcscpy(InitParamter.m_szServerDataBaseUser, _T("sa"));
	//_tcscpy(InitParamter.m_szServerDataBasePass, _T("123456@zxc"));
	...
	//信息数据库地址
	USES_CONVERSION;
	DWORD dwDataBaseIP=inet_addr(W2A(InitParamter.m_szServerDataBaseAddr));
	if (dwDataBaseIP==INADDR_NONE)
	{
		LPHOSTENT lpHost = gethostbyname(W2A(InitParamter.m_szServerDataBaseAddr));
...
void CSystemOptionDlg::OnOK()
{
	...
	GetDlgItemText(IDC_SERVER_DATABASE_USER,InitParamter.m_szServerDataBaseUser,ARRAYSIZE(InitParamter.m_szServerDataBaseUser));
	GetDlgItemText(IDC_SERVER_DATABASE_PASS,InitParamter.m_szServerDataBasePass, ARRAYSIZE(InitParamter.m_szServerDataBasePass));
	GetDlgItemText(IDC_SERVER_DATABASE_NAME,InitParamter.m_szServerDataBaseName, ARRAYSIZE(InitParamter.m_szServerDataBaseName));
	...
	_snwprintf(InitParamter.m_szServerDataBaseAddr, sizeof(InitParamter.m_szServerDataBaseAddr), TEXT("%d.%d.%d.%d"),
