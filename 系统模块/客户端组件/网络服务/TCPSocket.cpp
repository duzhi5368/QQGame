        ...
        else
        {
            ...
            if (iErrorCode == SOCKET_ERROR)
            {
                ...
                if (iErrorCode != WSAEWOULDBLOCK)
                {
                    ...
                    _snwprintf(szBuffer, sizeof(szBuffer), TEXT("连接发生错误，错误代码 [ %d ]"), iErrorCode);
                    throw szBuffer;
                }
            }
...
DWORD CTCPSocket::TranslateAddr(LPCTSTR pszServerAddr)
{
    //转化地址
    USES_CONVERSION;
    DWORD dwServerIP = inet_addr(W2A(pszServerAddr));
    if (dwServerIP == INADDR_NONE)
    {
        LPHOSTENT lpHost = gethostbyname(W2A(pszServerAddr));
        if (lpHost == NULL)
            return INADDR_NONE;
