		...
		case IDI_SEND_SYSTEM_MESSAGE:		//系统消息
			{
				...
				_snwprintf(szFileName, sizeof(szFileName), TEXT("%s\\GameService.ini"), szPath);
				
				//循环消息
				TCHAR szRoomMsgName[64],szGameMsgName[64];
				_snwprintf(szRoomMsgName, sizeof(szRoomMsgName), TEXT("RoomMessage%d"), m_cbRoomMsgIndex++);
				_snwprintf(szGameMsgName, sizeof(szGameMsgName), TEXT("GameMessage%d"), m_cbGameMsgIndex++);
...
bool CAttemperEngineSink::OnDBUserRightResult(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	else
	{
		...
		_snwprintf(szDescribe, sizeof(szDescribe), TEXT("设置用户权限失败！"));
				...
				case DTP_COMPUTER_ID:		//机器标识
					{
						...
						if (DataDescribe.wDataSize==sizeof(tagClientSerial))
						{
							...
							for (INT i=0;i<sizeof(tagClientSerial)/sizeof(BYTE);i++)
							{
                                ...
                                wsprintf(&LogonByUserID.szComputerID[i * 2], TEXT("%02X"), *(pcbByte + i));
							}
						}
	...
	case SUB_GR_USER_LOOKON_REQ:	//旁观桌子
		{
			...
			if ((wSubCmdID==SUB_GR_USER_SIT_REQ)&&(m_pGameServiceOption->cbHideUserInfo==TRUE))
			{
				...
				if (pUserData->wTableID!=INVALID_TABLE)
				{
					...
					if (pTableFrame->IsUserPlaying(pIServerUserItem)==true)
					{
						...
						_snwprintf(SitFailed.szFailedDescribe, sizeof(SitFailed.szFailedDescribe), TEXT("你正在的 [ %d ] 号游戏桌游戏，暂时不能离开位置！"), pUserData->wTableID + 1);
				...
				_snwprintf(SitFailed.szFailedDescribe, sizeof(SitFailed.szFailedDescribe), TEXT("暂时没有能够加入的游戏桌，请稍后！"));
			...
			if ((wSubCmdID==SUB_GR_USER_LOOKON_REQ)&&(m_pGameServiceOption->cbUnLookOnTag==TRUE))
			{
				...
				CMD_GR_SitFailed SitFailed;
				memset(&SitFailed,0,sizeof(SitFailed));
				_snwprintf(SitFailed.szFailedDescribe, sizeof(SitFailed.szFailedDescribe), TEXT("本房间限制了旁观游戏!"), pUserData->wTableID + 1);
	...
	case SUB_GR_USER_LEFT_GAME_REQ:	//强退请求
		{
			...
			if ((wSubCmdID==SUB_GR_USER_STANDUP_REQ)&&(pTableFrame->IsUserPlaying(pIServerUserItem)==true))
			{
				...
				_snwprintf(szMessage, sizeof(szMessage), TEXT("您正在的 [ %d ] 号游戏桌游戏，暂时不能离开位置！"), pUserData->wTableID + 1);
	...
	case SUB_GR_LOOK_USER_IP:	//查看地址
		{
			...
			_snwprintf(szMessage, CountArray(szMessage), TEXT("[ %s ]：%d.%d.%d.%d"), pIServerUserItemRecv->GetAccounts(),
	...
	if(m_pGameServiceOption->wServerType == GAME_GENRE_GOLD)
	{
		...
		if((pIServerUserItem->GetTableID()!=INVALID_TABLE) && (lLessScore>0) && (lLessScore>pServerUserData->UserScoreInfo.lScore))
		{
			...
			if (!(pTableFrame->IsUserPlaying(pIServerUserItem)))
			{
				...
				_snwprintf(szDescribe, sizeof(szDescribe), TEXT("加入游戏至少需要 %ld 的金币，您的金币不够，不能加入！"), lLessScore);
