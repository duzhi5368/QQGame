...
bool CBankItemDlg::CheckInput()
{
	...
	else
	{
		if (1==m_nSelectType)
		{
			...
			{
				TCHAR ch=m_strNickname[i];
                if (!_istdigit(ch))
				{
					AfxMessageBox(_T("请输入合法的游戏ID"));
					GetDlgItem(IDC_NICKNAME)->SetFocus();
					return false;
				}
			}
