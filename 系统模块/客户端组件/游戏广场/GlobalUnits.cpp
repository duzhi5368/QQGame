...
CGlobalUnits::CGlobalUnits(void)
{
    ...
    lstrcpyn(m_szStationPage, g_szStationPage, sizeof(m_szStationPage));
...
bool CGlobalUnits::InitGlobalUnits()
{
    ...
    //设置界面目录
    _snwprintf(m_szDirSkin, sizeof(m_szDirSkin), TEXT("%s\\Skin"), m_szDirWork);
...
bool CGlobalUnits::CopyToClipboard(LPCTSTR pszString)
{
    ...
    //复制数据
    LPWSTR pszMemString = (LPWSTR)GlobalLock(hData);
...
bool CGlobalUnits::WriteUserCookie()
{
    ...
    CInternetSession::SetCookie(g_szCookieUrl, TEXT("UserID"), szTokenData);

    //游戏 I D
    _sntprintf(szTokenData, nCharCount, TEXT("%ld"), m_GloblaUserData.dwGameID);
    CInternetSession::SetCookie(g_szCookieUrl, TEXT("GameID"), szTokenData);

    //用户帐号
    CInternetSession::SetCookie(g_szCookieUrl, TEXT("Accounts"), m_GloblaUserData.szAccounts);

    //用户密码
    CInternetSession::SetCookie(g_szCookieUrl, TEXT("Password"), m_GloblaUserData.szPassWord);
...
{
    //删除信息
    CInternetSession::SetCookie(g_szCookieUrl, TEXT("UserID"), TEXT(""));
    CInternetSession::SetCookie(g_szCookieUrl, TEXT("GameID"), TEXT(""));
    CInternetSession::SetCookie(g_szCookieUrl, TEXT("Accounts"), TEXT(""));
    CInternetSession::SetCookie(g_szCookieUrl, TEXT("Password"), TEXT(""));
    CInternetSession::SetCookie(g_szCookieUrl, TEXT("UserToken"), TEXT(""));
...
bool CGlobalAttemper::DownLoadClient(LPCTSTR pszKindName, WORD wKindID, bool bDisplay)
{
    ...
    _snwprintf(DownLoadRequest.szFileName, sizeof(DownLoadRequest.szFileName), TEXT("%s.EXE"), pszKindName);
    _snwprintf(DownLoadRequest.szLocalPath, sizeof(DownLoadRequest.szLocalPath), TEXT("%s\\DownLoad"), g_GlobalUnits.GetWorkDirectory());
    _snwprintf(DownLoadRequest.szDownLoadUrl, sizeof(DownLoadRequest.szDownLoadUrl), TEXT("http://221.234.25.72/Download.asp?KindID=%ld&LocalVersion=0&PlazaVersion=%ld"), wKindID, VER_PLAZA_FRAME);
