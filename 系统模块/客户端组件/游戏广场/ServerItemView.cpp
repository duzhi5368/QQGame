...
bool CServerListManager::InitServerListManager(IServerListSink * pIServerListSink)
{
    ...
    lstrcpyn(GameInside.szDisplayName, g_szProductName, sizeof(GameInside.szDisplayName));
...
bool CServerListManager::UpdateGameOnLineCount(DWORD dwOnLineCount)
{
    if (m_PtrArrayInside.GetCount() > 0)
    {
        ...
        _snwprintf(pGameInside->szDisplayName, sizeof(pGameInside->szDisplayName), TEXT("%s [ %ld 人]"), g_szProductName, dwOnLineCount);
...
{
    //FIXME: 奇怪,为什么点击"斗地主1"节点得到的tree节点都是NULL?
    //变量定义
    CListItem * pListItem = NULL;
    HTREEITEM hTreeItem = GetCurrentTreeItem();
...
LPCTSTR CServerItemView::GetGameItemTitle(CListKind * pListKind, LPTSTR pszTitle, WORD wBufferSize)
{
    ...
    if (m_bShowOnLineCount == true)
    {
        ...
        {
            _snwprintf(pszTitle, wBufferSize, TEXT("%s [ %ld ]"), pGameKind->szKindName, pGameKind->dwOnLineCount);
        }
        else
        {
            _snwprintf(pszTitle, wBufferSize, TEXT("%s [ %ld ] （双击下载）"), pGameKind->szKindName, pGameKind->dwOnLineCount);
        }
    }
    else
    {
        if (pListKind->m_bInstall == true)
        {
            _snwprintf(pszTitle, wBufferSize, TEXT("%s"), pGameKind->szKindName);
        }
        else
        {
            _snwprintf(pszTitle, wBufferSize, TEXT("%s （双击下载）"), pGameKind->szKindName);
        }
    }
...
LPCTSTR CServerItemView::GetGameItemTitle(CListServer * pListServer, LPTSTR pszTitle, WORD wBufferSize)
{
    ...
    if (m_bShowOnLineCount == true)
    {
        _snwprintf(pszTitle, wBufferSize, TEXT("%s [ %ld ]"), pGameServer->szServerName, pGameServer->dwOnLineCount);
    }
    else
    {
        _snwprintf(pszTitle, wBufferSize, TEXT("%s"), pGameServer->szServerName);
    }
