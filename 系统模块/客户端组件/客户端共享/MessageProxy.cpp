...
bool __cdecl CMessageProxy::InsertSystemTime(COLORREF crColor)
{
	...
	_snwprintf(szTimeBuffer, sizeof(szTimeBuffer), TEXT("［%02d:%02d:%02d］"), SystemTime.wHour, SystemTime.wMinute, SystemTime.wSecond);
...
bool CMessageProxy::TranslateInsert(LPCTSTR pszString, COLORREF crColor, UINT uSize, LPCTSTR pszFontName)
{
	...
	do
	{
		...
		if (nFaceIndex!=-1)
		{
			...
			//装载图片			
			_snwprintf(szFileName, sizeof(szFileName), TEXT("%s\\Face\\%d.gif"), m_szPath, nFaceIndex);
