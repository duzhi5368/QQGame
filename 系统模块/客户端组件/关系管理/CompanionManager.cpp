...
bool __cdecl CCompanionManager::LoadCompanion()
{
    //打开文件
    CFile CompanionFile;
    if (CompanionFile.Open(TEXT("Companion.DAT"), CFile::modeRead) == FALSE)
        return true;

    //判断大小
    ULONGLONG uFileSize = CompanionFile.GetLength();
    if (uFileSize < sizeof(tagCompanionFileHead))
        return true;
    ...
    //效验文件头
    if (HIBYTE(CompanionFileHead.wVersion) != VER_COMPANION_MAIN)
        return false;
    ...
    if (CompanionFileHead.dwFileSize != uFileSize)
        return false;
    if (((CompanionFileHead.dwFileSize - sizeof(CompanionFileHead)) % CompanionFileHead.wItemSize) != 0)
        return false;
    ...
    for (DWORD i = 0; i < dwItemCount; i++)
    {
        ...
        //插入关系
        if (SearchCompanionItem(pCompanionItem->dwUserID) != NULL)
