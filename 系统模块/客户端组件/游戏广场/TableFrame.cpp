...
bool CTableResource::LoadFromFiles(bool bHideInfo,LPCTSTR pszGameName, WORD wChairCount, WORD wKindID)
{
	try
	{
		...
		_snwprintf(szResourcePath, sizeof(szResourcePath), TEXT("%s%s\\TableFrame.DAT"), g_GlobalUnits.GetWorkDirectory(), pszGameName);
		...
		{
			m_bDZShowHand=true;
			_snwprintf(szResourcePath, sizeof(szResourcePath), TEXT("%s%s\\Player.BMP"), g_GlobalUnits.GetWorkDirectory(), pszGameName);
			m_ImagePeople.Load(szResourcePath);
			if (m_ImagePeople.IsNull()) throw 0;
		}

		//加载锁位图
		_snwprintf(szResourcePath, sizeof(szResourcePath), TEXT("%s%s\\Locker.BMP"), g_GlobalUnits.GetWorkDirectory(), pszGameName);
		...
		//加载准备标志
		_snwprintf(szResourcePath, sizeof(szResourcePath), TEXT("%s%s\\UserReady.BMP"), g_GlobalUnits.GetWorkDirectory(), pszGameName);
		...
		//加载开始标志
		_snwprintf(szResourcePath, sizeof(szResourcePath), TEXT("%s%s\\TablePlaying.BMP"), g_GlobalUnits.GetWorkDirectory(), pszGameName);
		m_ImagePlay.Load(szResourcePath);
		if (m_ImagePlay.IsNull()) throw 0;

		//加载桌子位图
		_snwprintf(szResourcePath, sizeof(szResourcePath), TEXT("%s%s\\TableNormal.BMP"), g_GlobalUnits.GetWorkDirectory(), pszGameName);
...
void __cdecl CTableView::DrawTable(CDC * pDC, bool bHideInfo, CTableResource * pTableResource, bool bShowUser)
{
	...
	if (m_TableState.szTableNote[0]==0)
	{
		...
		_snwprintf(szBuffer, sizeof(szBuffer), TEXT("- %d -"), m_TableState.wTableID + 1);
