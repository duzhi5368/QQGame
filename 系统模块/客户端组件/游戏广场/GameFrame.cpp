	...
	case ItemGenre_Kind:		//游戏类型
		{
			...
			_snwprintf(szRuleUrl, sizeof(szRuleUrl), TEXT("http://221.234.25.72/GameRule.asp?KindID=%ld"), pGameKind->wKindID);
	...
	case ItemGenre_Kind:	//游戏类型
		{
			...
			if ((pListKind->m_bInstall==false)&&(pListKind->m_GameKind.dwMaxVersion!=0L))
			{
				...
				_snwprintf(szBuffer, sizeof(szBuffer), TEXT("【%s】还没有安装，现在是否下载？"), pGameKind->szKindName);
...
CRoomViewItem * CGameFrame::CreateRoomViewItem(CListServer * pListServer)
{
 ...
 //           g_GlobalAttemper.DownLoadClient(pGameKind->szKindName,pGameKind->wKindID,true);
	...
	//{
	//	TCHAR szBuffer[512]=TEXT("");
	//	_snwprintf(szBuffer, sizeof(szBuffer), TEXT("【%s】已经更新为 %ld.%ld.%ld.%ld 版本，你的版本不能继续使用，现在是否下载？"), pGameKind->szKindName,
 ...
 //           g_GlobalAttemper.DownLoadClient(pGameKind->szKindName,pGameKind->wKindID,true);
	...
	//{
	//	TCHAR szBuffer[512]=TEXT("");
	//	_snwprintf(szBuffer, sizeof(szBuffer), TEXT("【%s】已经更新为 %ld.%ld.%ld.%ld 版本，现在是否下载升级？"), pGameKind->szKindName,
	//		cbListVer1,cbListVer2,cbListVer3,cbListVer4);
	//	int nResult=ShowMessageBox(szBuffer,MB_ICONQUESTION|MB_YESNO|MB_DEFBUTTON1);
	//	if (nResult==IDYES) 
	//	{
...
int CGameFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	//创建控件
    //在m_DlgGamePlaza的OnCreate()函数中产生登录对话框
