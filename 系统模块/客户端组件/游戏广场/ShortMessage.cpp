...
BOOL CShortMessage::OnInitDialog()
{
	...
	{
		TCHAR szTitle[256];
		_snwprintf(szTitle, sizeof(szTitle), TEXT("[ %s ] － [ %s ]"), m_pTargetUserItem->GetUserData()->szName, m_strRoomName);
	...
	_snwprintf(szTitle, CountArray(szTitle), TEXT("【温馨提示】"));
	m_MessageProxyHelper->InsertGeneralString(szTitle,RGB(255,0,0),MS_NORMAL,false);

	//预警信息
	TCHAR szMessage[128] ;
	_snwprintf(szMessage, CountArray(szMessage), TEXT("客服人员不会以任何形式向您索取您的帐户信息，请玩家提高警惕，保护好自己帐号密码！"));
...
void CShortMessage::SetTalkInfo(IUserItem * pTargerUserItem, TCHAR * szRoomName)
{
	...
	if (GetSafeHwnd()!=NULL)
	{
		//设置标题
		TCHAR szTitle[256];
		_snwprintf(szTitle, sizeof(szTitle), TEXT("[ %s ] － [ %s ]"), m_strTargerName, m_strRoomName);
		SetWindowText(szTitle);
	...
	{
		//结尾判断
		if(szPath[wcslen(szPath)-1]!='\\') _tcscat(szPath, TEXT("\\")) ;
		...
		//完整目录
		_tcscat(szPath, TEXT("聊天记录\\"));
		_tcscat(szPath, strUserID);
	...
	_tcscat(szPath, strTargetUserID);
...
void CShortMessage::OnRecvMessage(TCHAR * szSendName, TCHAR * szMessage, COLORREF crColor, bool bMeMessage)
{
	...
	{
		TCHAR szTitle[128]=TEXT("");
		_snwprintf(szTitle, CountArray(szTitle), TEXT("[%s] %u/%u/%u %02d:%02d:%02d"), szSendName, SystemTime.wYear, SystemTime.wMonth, SystemTime.wDay, SystemTime.wHour, SystemTime.wMinute, SystemTime.wSecond);
		...
		{
			MsgFile.SeekToEnd() ;
			MsgFile.Write(szTitle, UINT(wcslen(szTitle))) ;
			MsgFile.Write("\r\n", UINT(strlen("\r\n"))) ;
		
			MsgFile.Write(szMessage, UINT(wcslen(szMessage)));
			MsgFile.Write("\r\n\r\n", UINT(strlen("\r\n\r\n"))) ;
			MsgFile.Close() ;
		}
...
void CShortMessage::OnBnClickedOpenChatMsg()
{
	try
	{
		//打开记录
		TCHAR szBuffer[128] ;
		_tcscpy(szBuffer, m_strMsgFileName) ;
