...
BOOL CDlgOptionTableRule::OnInitDialog()
{
	...
	//最低胜率
	_snwprintf(szBuffer, sizeof(szBuffer), TEXT("%.2f"), ((double)(m_TableRuleParameter.wWinRate)) / 100.0);
	SetDlgItemText(IDC_WIN_RATE,szBuffer);

	//最高逃率
	_snwprintf(szBuffer, sizeof(szBuffer), TEXT("%.2f"), ((double)(m_TableRuleParameter.wFleeRate)) / 100.0);
...
bool CDlgOptionTableRule::SaveOptions()
{
	...
	m_TableRuleParameter.wWinRate=(WORD)(_wtof(szBuffer)*100.0)%10000;

	//最高逃率
	GetDlgItemText(IDC_FLEE_RATE,szBuffer,CountArray(szBuffer));
	m_TableRuleParameter.wFleeRate = (WORD)(_wtof(szBuffer)*100.0) % 10000;
