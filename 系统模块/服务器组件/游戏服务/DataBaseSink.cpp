...
bool __cdecl CDataBaseSink::OnDataBaseEngineStart(IUnknownEx * pIUnknownEx)
{
	...
	try
	{
		...
		_snwprintf(szDataBaseAddr, ARRAYSIZE(szDataBaseAddr), TEXT("%d.%d.%d.%d"), pcbAddr[0], pcbAddr[1], pcbAddr[2], pcbAddr[3]);
        ...
        _snwprintf(szDataBaseAddr, ARRAYSIZE(szDataBaseAddr), TEXT("%d.%d.%d.%d"), pcbAddr[0], pcbAddr[1], pcbAddr[2], pcbAddr[3]);
		m_GameScoreDBModule->SetConnectionInfo(szDataBaseAddr, m_pGameScoreDBInfo->wDataBasePort, _T("QPGameScoreDB")/*m_pGameScoreDBInfo->szDataBaseName*/,
