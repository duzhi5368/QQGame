...
BOOL CDlgRegister::OnInitDialog()
{
    ...
    _snwprintf(szFileName, sizeof(szFileName), TEXT("%s\\Spreader.ini"), szPath);
...
BOOL CDlgLogon::OnInitDialog()
{
    ...
    m_BrowerAD.Navigate(TEXT("http://www.hootina.org"), NULL, NULL, NULL, NULL);
    ...
    if ((m_LogonMode != LogonMode_Accounts) && (m_LogonMode != LogonMode_UserID))
        m_LogonMode = LogonMode_Accounts;
...
bool CDlgLogon::SendLogonPacket(ITCPSocket * pIClientSocke)
{
    //变量定义
    TCHAR szPassword[33];
    //SOCKET_PACKET = SOCKET_BUFFER-sizeof(CMD_Head)-2*sizeof(DWORD)
    //即SOCKET_PACKET = 8192-sizeof(CMD_Head)-2*sizeof(DWORD)
    //2个DWORD大小是tagDataDescribe大小
...
bool CDlgLogon::OnLogonSuccess()
{
    ...
    if (g_GlobalOption.m_enAcountsRule != enAcountsRule_None)
    {
        //构造信息
        TCHAR szBuffer[256];
        _snwprintf(szBuffer, sizeof(szBuffer), TEXT("%s\\%ld"), REG_USER_INFO, UserData.dwUserID);
        ...
        if (RegUserInfo.Create(HKEY_CURRENT_USER, szBuffer) == ERROR_SUCCESS)
        {
            ...
            if (bRemPassword)
                RegUserInfo.SetKeyValue(TEXT(""), m_szPassword, TEXT("UserPassword"));
    ...
    else
    {
        //构造信息
        TCHAR szBuffer[256];
        _snwprintf(szBuffer, sizeof(szBuffer), TEXT("%ld"), UserData.dwUserID);
...
void CDlgLogon::LoadAccountsInfo()
{
    ...
    if (RegUserInfo != NULL)
    {
        ...
        do
        {
            ...
            dwUserDBID = _wtol(szUserIDBuffer);
            if (dwUserDBID == 0) continue;

            //加载其他信息
            CRegKey RegUserAccount;
            _snwprintf(szTempBuffer, sizeof(szTempBuffer), TEXT("%s\\%ld"), REG_USER_INFO, dwUserDBID);
...
void CDlgLogon::LoadLogonServer()
{
    ...
    {
        pComBoxServer->AddString(SERVER_ADDR);
