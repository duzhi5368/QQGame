...
BOOL CSystemOptionDlg::OnInitDialog()
{
    ...
    //登录数据库地址
    DWORD dwDataBaseIP = inet_addr(CStringA(CStringW(InitParamter.m_szUserDataBaseAddr)));
    if (dwDataBaseIP == INADDR_NONE)
    {
        LPHOSTENT lpHost = gethostbyname(CStringA(CStringW(InitParamter.m_szUserDataBaseAddr)));
    ...
    //主站地址
    if (InitParamter.m_szMainPage[0] == 0) 
        SetDlgItemText(IDC_MAIN_PAGE, g_szStationPage);
    else 
        SetDlgItemText(IDC_MAIN_PAGE, InitParamter.m_szMainPage);

    //中心服务器
    dwDataBaseIP = inet_addr(CStringA(CStringW(InitParamter.m_szCenterServerAddr)));
    if (dwDataBaseIP == INADDR_NONE)
    {
        LPHOSTENT lpHost = gethostbyname(CStringA(CStringW(InitParamter.m_szCenterServerAddr)));
        if (lpHost != NULL) dwDataBaseIP = ((LPIN_ADDR)lpHost->h_addr)->s_addr;
    }
...
void CSystemOptionDlg::OnOK()
{
    ...
    _snwprintf(InitParamter.m_szUserDataBaseAddr, sizeof(InitParamter.m_szUserDataBaseAddr), TEXT("%d.%d.%d.%d"),
    ...
    if (lstrcmp(InitParamter.m_szMainPage, g_szStationPage) == 0) InitParamter.m_szMainPage[0] = 0;
