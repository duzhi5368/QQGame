...
bool __cdecl CTableFrame::InitTableFrame(WORD wTableID, tagTableFrameParameter * pTableFrameParameter)
{
	...
	if (m_pITableFrameSink->InitTableFrameSink(pIUnknownEx)==false) 
        return false;
...
bool __cdecl CTableFrame::PerformSitDownAction(WORD wChairID, IServerUserItem * pIServerUserItem, LPCTSTR szPassword)
{
	...
	if (m_pIUserItem[wChairID]!=NULL)
	{
		...
		if ((m_pIUserItem[wChairID]->GetChairID()==wChairID)&&(m_pIUserItem[wChairID]->GetTableID()==m_wTableID))
		{
			...
			_snwprintf(szDescribe, sizeof(szDescribe), TEXT("椅子已经被 [ %s ] 捷足先登了，下次动作要快点了！"), pTableUserData->szAccounts);
	...
	else
	{
		...
		if (lLessScore!=0L || lMaxScore>lLessScore)
		{
			if (pUserData->UserScoreInfo.lScore<lLessScore)
			{
				...
				{
					_snwprintf(szDescribe, sizeof(szDescribe), TEXT("加入游戏至少需要 %ld 个金币，您的金币不够，不能加入！"), lLessScore);
				}
				else
				{
					_snwprintf(szDescribe, sizeof(szDescribe), TEXT("加入游戏至少需要 %ld 个游戏积分，您的积分不够，不能加入！"), lLessScore);
				}
				SendSitFailedPacket(pIServerUserItem,szDescribe);
			...
			if (lMaxScore>lLessScore && lMaxScore<pUserData->UserScoreInfo.lScore)
			{
				...
				if (m_pGameServiceOption->wServerType==GAME_GENRE_GOLD)
				{
					_snwprintf(szDescribe, sizeof(szDescribe), TEXT("您的金币超过本房间的最大限制额度%ld,请更换游戏房间!"), lMaxScore);
				}
				else
				{
					_snwprintf(szDescribe, sizeof(szDescribe), TEXT("您的游戏积分超过本房间的最大限制额度%ld,请更换游戏房间!"), lMaxScore);
				}
				SendSitFailedPacket(pIServerUserItem,szDescribe);
		...
		for (WORD i=0;i<m_wChairCount;i++)
		{
			if (m_pIUserItem[i]!=NULL)
			{
				...
				{
					TCHAR szDescribe[128]=TEXT("");
					_snwprintf(szDescribe, sizeof(szDescribe), TEXT("你的逃跑率太高，与 %s 设置的设置不符，不能加入游戏！"), m_pIUserItem[i]->GetAccounts());
					SendSitFailedPacket(pIServerUserItem,szDescribe);
					return false;
				}
				...
				{
					TCHAR szDescribe[128]=TEXT("");
					_snwprintf(szDescribe, sizeof(szDescribe), TEXT("你的胜率太低，与 %s 设置的设置不符，不能加入游戏！"), m_pIUserItem[i]->GetAccounts());
					SendSitFailedPacket(pIServerUserItem,szDescribe);
					return false;
				}
				...
				if (pTableUserRule->bLimitScore==true)
				{
					...
					{
						TCHAR szDescribe[128]=TEXT("");
						_snwprintf(szDescribe, sizeof(szDescribe), TEXT("你的积分太高，与 %s 设置的设置不符，不能加入游戏！"), m_pIUserItem[i]->GetAccounts());
						SendSitFailedPacket(pIServerUserItem,szDescribe);
						return false;
					}
					...
					{
						TCHAR szDescribe[128]=TEXT("");
						_snwprintf(szDescribe, sizeof(szDescribe), TEXT("你的积分太低，与 %s 设置的设置不符，不能加入游戏！"), m_pIUserItem[i]->GetAccounts());
						SendSitFailedPacket(pIServerUserItem,szDescribe);
						return false;
					}
				}
		...
		case SUB_GF_KICK_TABLE_USER:	//踢走用户
			{
				...
				{	
					...
					if (bKickUser==true)
					{
						...
						{
							TCHAR szDescribe[128]=TEXT("");
							_snwprintf(szDescribe, sizeof(szDescribe), TEXT("[ %s ] 使用了防踢卡，踢人失败！"), pKickUserData->szAccounts);
							SendGameMessage(pIServerUserItem,szDescribe,SMT_EJECT|SMT_INFO);

							_snwprintf(szDescribe, sizeof(szDescribe), TEXT("由于您使用了防踢卡，成功阻止了[ %s ]对您的踢人动作！"), pUserData->szAccounts);
						...
						TCHAR szDescribe[128]=TEXT("");
						_snwprintf(szDescribe, sizeof(szDescribe), TEXT("您被 [ %s ] 踢出游戏桌！"), pUserData->szAccounts);
		...
		for (WORD i=0;i<m_wChairCount;i++)
		{
			if (m_pIUserItem[i]!=NULL)
			{
				...
				if (pUserData->UserScoreInfo.lScore<lLessScore)
				{
					...
					if (m_pGameServiceOption->wServerType==GAME_GENRE_GOLD)
					{
						_snwprintf(szDescribe, sizeof(szDescribe), TEXT("您的金币少于 %ld，不能继续游戏！"), lLessScore);
					}
					else 
					{
						_snwprintf(szDescribe, sizeof(szDescribe), TEXT("您的游戏积分少于 %ld，不能继续游戏！"), lLessScore);
					}
					SendGameMessage(pIServerUserItem,szDescribe,SMT_EJECT|SMT_INFO|SMT_CLOSE_GAME);
...
bool __cdecl CTableFrame::SendGameScene(IServerUserItem * pIServerUserItem, void * pData, WORD wDataSize)
{
	...
	if (m_pGameServiceOption->wServerType==GAME_GENRE_MATCH)
	{
		_snwprintf(szMessage, sizeof(szMessage), TEXT("亲爱的用户，欢迎您来到“%s”比赛游戏房间，祝比赛选手赛出水平，取得好的成绩！"), m_pGameServiceAttrib->szKindName);
		SendGameMessage(pIServerUserItem,szMessage,SMT_INFO);
	}
	else
	{
		_snwprintf(szMessage, sizeof(szMessage), TEXT("亲爱的用户，欢迎您来到“%s”，欢迎你多提宝贵建议！"), m_pGameServiceAttrib->szKindName);
		SendGameMessage(pIServerUserItem,szMessage,SMT_INFO);
	}
