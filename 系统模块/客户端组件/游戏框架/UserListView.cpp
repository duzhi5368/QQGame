	...
	if (lpszCmdLine[0]!=0)
	{
		...
		while (true)
		{
			...
			if ((nStringLength>=nTokenLength)&&(memcmp(lpszBeginString,pszRoomToken,nTokenLength*sizeof(TCHAR))==0))
			{
				...
				wGameType=(WORD)_wtol(strRoomToken);
				break;
			}
...
LPCTSTR CUserListView::GetDisplayText(tagUserData * pUserData, WORD wColumnPos)
{
	...
	case DTP_USER_ID:			//用户 ID
		{
			_snwprintf(szBuffer,sizeof(szBuffer),TEXT("%ld"),pUserData->dwUserID);
			return szBuffer;
		}
	case DTP_GAME_ID:			//游戏 ID
		{
			_snwprintf(szBuffer,sizeof(szBuffer),TEXT("%ld"),pUserData->dwGameID);
			return szBuffer;
		}
	case DTP_INSURE_SCORE:		//存储金币
		{
			_snwprintf(szBuffer,sizeof(szBuffer),TEXT("%ld"),pUserData->lInsureScore);
			return szBuffer;
		}
	case DTP_GAME_GOLD:			//游戏金币
		{
			_snwprintf(szBuffer,sizeof(szBuffer),TEXT("%ld"),pUserData->lGameGold);
			return szBuffer;
		}
	case DTP_LOVE_LINESS:		//用户魅力
		{
			_snwprintf(szBuffer,sizeof(szBuffer),TEXT("%ld"),pUserData->lLoveliness);
			return szBuffer;
		}
	...
	case DTP_USER_PLAY_COUNT:	//总局盘数
		{
			...
			_snwprintf(szBuffer, sizeof(szBuffer), TEXT("%ld"), lAllCount);
			return szBuffer;
		}
