	...
	case IDM_UM_SET_DETEST:		//设为厌恶
		{
			...
			if (pUserData->cbCompanion==enCompanion_Detest)
			{
				...
				_snwprintf(szMessage, CountArray(szMessage), TEXT("[ %s ] 设置为厌恶玩家，系统将屏蔽他的一切聊天消息"), pUserData->szName);
	...
	case IDM_UM_LIMIT_ACCOUN:	//禁用帐户
		{
			...
			_snwprintf(szMessage, CountArray(szMessage), TEXT("确实要冻结 [ %s ] 的帐号吗？"), pIUserItem->GetUserName());
			if (ShowMessageBox(szMessage,MB_ICONQUESTION|MB_YESNO)!=IDYES) return TRUE;

			//用户判断
			if (pIUserItem->GetUserID()!=m_dwMenuUserID) 
			{
				_snwprintf(szMessage, CountArray(szMessage), TEXT("用户 [ %s ] 已经离开了游戏房间，帐号冻结失败"), pIUserItem->GetUserName());
				m_MessageProxyHelper->InsertSystemString(szMessage,0);
...
bool CRoomViewItem::InitRoomViewItem(CListServer * pListServer)
{
	...
	_snwprintf(m_szShareName, sizeof(m_szShareName), TEXT("0x8%ld%ld%ld%ld"), pGameServer->dwServerAddr, pGameServer->wServerPort, time(NULL), rand() % 100);
...
bool CRoomViewItem::ConnectGameServer()
{
    ...
    USES_CONVERSION;
    //实际调用的是CTCPSocket::Connect
    return (m_ClientSocket->Connect(SERVER_ADDR/*pGameServer->dwServerAddr*/, PORT_CENTER_SERVER) == CONNECT_SUCCESS);
}
...
bool CRoomViewItem::ShowUserInfoMenu(IUserItem * pIUserItem, CPoint Point)
{
	...
	{
		_snwprintf(szBuffer, CountArray(szBuffer), TEXT("与 [ %s ] 交谈"), pUserData->szName);
		UserInfoMenu.AppendMenu(0,IDM_UM_SET_CHAT,szBuffer);
	}
	if (m_szChatName[0]!=0)
	{
		_snwprintf(szBuffer, CountArray(szBuffer), TEXT("取消与 [ %s ] 交谈"), m_szChatName);
	...
	{
		_snwprintf(szBuffer, CountArray(szBuffer), TEXT("邀请 [ %s ] 一起游戏"), pUserData->szName);
	...
	if ((m_cbHideUserInfo==FALSE)&&(pUserData->cbMemberOrder>0)&&(pUserData->cbMemberOrder<CountArray(pszMember)))
	{
		...
		_snwprintf(szBuffer, CountArray(szBuffer), TEXT("用户名：%s [ %s ]"), pUserData->szName, pszMember[cbMemberOrder]);
	}
	else _snwprintf(szBuffer, CountArray(szBuffer), TEXT("用户名：%s"), pUserData->szName);
	UserInfoMenu.AppendMenu(0,0,szBuffer);

	//游戏 ID
	_snwprintf(szBuffer, CountArray(szBuffer), TEXT("游戏 ID：%ld"), pUserData->dwGameID);
	UserInfoMenu.AppendMenu(0,0,szBuffer);
	
	//个性签名
	if (pUserData->szUnderWrite[0]!=0)
	{
		_snwprintf(szBuffer, CountArray(szBuffer), TEXT("个性签名：%s"), pUserData->szUnderWrite);
		UserInfoMenu.AppendMenu(0,0,szBuffer);
	}
	...
	//经验数值
	_snwprintf(szBuffer, CountArray(szBuffer), TEXT("经验值：%ld"), pUserData->lExperience);
	UserInfoMenu.AppendMenu(0,0,szBuffer);
	
	//玩家位置
	if (pUserData->wTableID!=INVALID_TABLE)
	{
		_snwprintf(szBuffer, CountArray(szBuffer), TEXT("桌号：%d"), pUserData->wTableID + 1);
		UserInfoMenu.AppendMenu(0,0,szBuffer);
	}

	//输赢信息
	_snwprintf(szBuffer, CountArray(szBuffer), TEXT("总局数：%ld  赢局：%ld  输局：%ld  和局：%ld"), lPlayCount, pUserData->lWinCount, pUserData->lLostCount, pUserData->lDrawCount);
	UserInfoMenu.AppendMenu(0,0,szBuffer);

	//积分信息
	if (m_wGameGenre==GAME_GENRE_GOLD)
	{
		_snwprintf(szBuffer, CountArray(szBuffer), TEXT("游戏币：%ld  逃跑率：%5.2f%%"), pUserData->lScore, dFleeRate);
			UserInfoMenu.AppendMenu(0,0,szBuffer);
	}
	else
	{
		_snwprintf(szBuffer, CountArray(szBuffer), TEXT("积分：%ld  总局数：%ld  逃跑率：%5.2f%%"), pUserData->lScore, lPlayCount, dFleeRate);
			UserInfoMenu.AppendMenu(0,0,szBuffer);
	}
	...
	{
		_snwprintf(szBuffer, CountArray(szBuffer), TEXT("社团：%s"), pUserData->szGroupName);
		UserInfoMenu.AppendMenu(0,0,szBuffer);
	}
...
void CRoomViewItem::DrawTableFrame(CDC * pDC)
{
	...
	CListKind * pListKind=m_pListServer->GetListKind();
	_snwprintf(szRoomTitle, sizeof(szRoomTitle), TEXT("[ %s ] - [ %s ] （%ld 人）"), pListKind->GetItemInfo()->szKindName,
	...
	for (WORD i=0;i<m_TableFrame.GetTableCount();i++)
	{
		...
		if (g_GlobalOption.m_bLimitDetest==true)
		{
			if (pTableUserData->cbCompanion==enCompanion_Detest)
			{
				if (bShowError==true)
				{
					...
					_snwprintf(szDescribe, sizeof(szDescribe), TEXT("你设置了不与不受欢迎的玩家游戏，此桌有你不欢迎的玩家 [ %s ] ！"), pTableUserData->szName);
		...
		if (m_pGameOption->m_bLimitWin)
		{
			...
			if (wWinRate<m_pGameOption->m_wWinRate)
			{
				...
				{
					TCHAR szDescribe[128]=TEXT("");
					_snwprintf(szDescribe, sizeof(szDescribe), TEXT("[ %s ] 的胜率太低了，与你的设置不符！"), pTableUserData->szName);
					ShowMessageBox(szDescribe,MB_ICONINFORMATION);
				}
				return false;
			}
		...
		if (m_pGameOption->m_bLimitFlee)
		{
			...
			if ((wFillRate>0)&&(wFillRate>m_pGameOption->m_wFleeRate))
			{
				...
				{
					TCHAR szDescribe[128]=TEXT("");
					_snwprintf(szDescribe, sizeof(szDescribe), TEXT("[ %s ] 的逃跑率太高了，与你的设置不符！"), pTableUserData->szName);
					ShowMessageBox(szDescribe,MB_ICONINFORMATION);
				}
				return false;
			}
		...
		if (m_pGameOption->m_bLimitScore)
		{
			...
			if (lTableUserScore>m_pGameOption->m_lMaxScore)
			{
				...
				{
					TCHAR szDescribe[128]=TEXT("");
					_snwprintf(szDescribe, sizeof(szDescribe), TEXT("[ %s ] 的积分太高了，与你的设置不符！"), pTableUserData->szName);
					ShowMessageBox(szDescribe,MB_ICONINFORMATION);
				}
				return false;
			}
			if (lTableUserScore<m_pGameOption->m_lLessScore)
			{
				...
				{
					TCHAR szDescribe[128]=TEXT("");
					_snwprintf(szDescribe, sizeof(szDescribe), TEXT("[ %s ] 的积分太低了，与你的设置不符！"), pTableUserData->szName);
					ShowMessageBox(szDescribe,MB_ICONINFORMATION);
				}
				return false;
			}
...
bool __cdecl CRoomViewItem::OnEventTCPSocketShut(WORD wSocketID, BYTE cbShutReason)
{
	...
	if (cbShutReason!=SHUT_REASON_NORMAL)
	{
		...
		{
			m_MessageProxyHelper->InsertSystemString(_T("由于网络问题，您已经与服务器断开连接，请重新连接"),0);
			ShowMessageBox(_T("由于网络问题，您已经与服务器断开连接，请重新连接"),MB_ICONINFORMATION);
		}
		else
		{
			ShowMessageBox(_T("由于网络数据包处理失败，网络中断了"),MB_ICONINFORMATION);
		}
	}
...
bool CRoomViewItem::OnSocketSubUserCome(CMD_Command Command, void * pData, WORD wDataSize)
{
	...
	if (pIUserItem!=NULL)
	{
		...
		if ((m_cbHideUserInfo==FALSE)&&(m_ServiceStatus==ServiceStatus_Serviceing))
		{
			...
			{
				TCHAR szMessage[256]=TEXT("");
				_snwprintf(szMessage, sizeof(szMessage), TEXT("[ %s ] 进来了"), UserData.szName);
				m_MessageProxyHelper->InsertSystemString(szMessage,0);
			}
...
bool CRoomViewItem::OnSocketSubUserInvite(CMD_Command Command, void * pData, WORD wDataSize)
{
	...
	//提示消息
	TCHAR szMessage[256]=TEXT("");
	_snwprintf(szMessage, sizeof(szMessage), TEXT("[ %s ] 邀请你加入第 [ %ld ] 游戏桌进行游戏，是否同意？"), pUserData->szName, pUserInvite->wTableID + 1);
...
void __cdecl CRoomViewItem::OnUserItemDelete(IUserItem * pIUserItem)
{
	...
	if (m_dwChatUserID==dwUserID)
	{
		SetChatObject(NULL);
		TCHAR szMessage[256]=TEXT("");
		_snwprintf(szMessage, sizeof(szMessage), TEXT("[ %s ] 离开了，聊天对象设置为所有人"), pIUserItem->GetUserName());
		m_MessageProxyHelper->InsertSystemString(szMessage,0);
	}
