...
void CServiceParameter::OnOK()
{
	...
	if (nCurSel==CB_ERR)
	{
		...
		OptionParameter.GameServiceOption.dwServerAddr=0/*inet_addr(szServerAddr)*/;
	}
	...
	_snwprintf(OptionParameter.szDataBaseAddr, sizeof(OptionParameter.szDataBaseAddr), TEXT("%d.%d.%d.%d"),
...
bool CServiceParameter::LoadGameServiceModule(LPCTSTR pszModuleName)
{
	...
	//查找函数
	ModuleCreateProc * CreateFunc=(ModuleCreateProc *)GetProcAddress(m_hDllInstance,"CreateGameServiceManager");
	if (CreateFunc==NULL)
	{
		FreeGameServiceModule();
		return false;
...
void CServiceParameter::UpdateOptionParameter()
{
	...
	if (m_OptionParameter.GameServiceOption.szGameRoomName[0]==0) 
	{
		...
		_snwprintf(szServerName, sizeof(szServerName), TEXT("%s房间"), m_GameServiceAttrib.szKindName);
	...
	if (nItemCount==i)
	{
		...
		_snwprintf(szServerAddr, CountArray(szServerAddr), TEXT("%ld.%ld.%ld.%ld"), cbServerAddr[0], cbServerAddr[1], cbServerAddr[2], cbServerAddr[3]);
	...
	//数据库地址
	USES_CONVERSION;
	DWORD dwDataBaseAddr=ntohl(inet_addr(W2A(m_OptionParameter.szDataBaseAddr)));
