...
void CGameClientView::DrawGameView(CDC * pDC, int nWidth, int nHeight)
{
	...
	for (WORD i=0;i<GAME_PLAYER;i++)
	{
		...
		//	if (i!=1)
			{
				_snwprintf(szBuffer,sizeof(szBuffer),TEXT("剩余：%d 张"),m_bCardCount[i]);
	...
	if (m_wLandUser!=INVALID_CHAIR)
	{
		...
		if (pUserData!=NULL)
		{
			...
			_snwprintf(szBuffer,sizeof(szBuffer),TEXT("地主：%s"),pUserData->szName);
			...
			_snwprintf(szBuffer,sizeof(szBuffer),TEXT("分数：%ld 分"),lScore);
	...
	if (m_wBombTime>1)
	{
		...
		//炸弹倍数
		TCHAR szBuffer[64];
		_snwprintf(szBuffer,sizeof(szBuffer),TEXT("炸弹翻倍：%d"),m_wBombTime);
	...
	if (m_bLandTitle==true)
	{
		...
		//绘画信息
		pDC->TextOut(nWidth/2,200,_T("正等待用户叫分"));
